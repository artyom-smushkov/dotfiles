...
complete -c gpg -l use-agent -d 'TQ   --no-use-agent This is dummy option.  gpg2 always requires the agent'
complete -c gpg -l gpg-agent-info -d 'This is dummy option.  It has no effect when used with gpg2'
...
complete -c gpg -l set-legacy-fips -d 'TQ   --set-legacy-fips Enable legacy support even when the libgcrypt library …'
...
complete -c gpg -l passphrase-repeat -d 'Specify how many times gpg2 will request a new passphrase be repeated'
...
complete -c gpg -l no-use-agent -d 'This is dummy option.  gpg2 always requires the agent'
